// app/demo/user/components/UserSidebar.client.tsx
"use client";

import Link from "next/link";
import { useMemo } from "react";
import { usePathname } from "next/navigation";
import { groups } from "./userNav.data";
...
const sidebarShell =
  "hidden xl:flex xl:w-[300px] xl:flex-col xl:pt-2";
const sidebarInner =
  "sticky top-[5.2rem] flex-1 overflow-y-auto pb-24 pr-2";

const navLink =
  "flex items-center gap-2 rounded-xl border px-3 py-2 text-sm transition";
const navMuted =
  "border-transparent text-white/70 hover:border-white/10 hover:bg-white/12";
const navActive =
...
export default function UserSidebar() {
    ...
    () =>
      (groups as Group[]).map((group, gi) => (
        <div key={`${group.title}::${gi}`} className="grid gap-2">
          <div className="px-2 text-xs uppercase tracking-wide text-white/60">
            {group.title}
          </div>
  ...
  return (
    <aside className={sidebarShell} aria-label="Боковая навигация пользователя">
