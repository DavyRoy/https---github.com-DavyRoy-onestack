// app/demo/user/components/UserMobileTabbar.client.tsx
"use client";
...
const TABS: Tab[] = [
  { href: "/demo/user/dashboard", label: "Дашборд", icon: "layout-grid" },
  { href: "/demo/user/shop", label: "Магазин", icon: "shopping-bag" },
  { href: "/demo/user/shop/products", label: "Товары", icon: "boxes" },
  { href: "/demo/user/cart", label: "Корзина", icon: "shopping-cart" },
  { href: "/demo/user/services", label: "Услуги", icon: "sparkles" },
  { href: "/demo/user/booking", label: "Запись", icon: "calendar-days" },
  { href: "/demo/user/calendar", label: "Календарь", icon: "calendar" },
  { href: "/demo/user/my-bookings", label: "Мои записи", icon: "notebook-pen" },
  { href: "/demo/user/my-orders", label: "Мои заказы", icon: "receipt" },
  { href: "/demo/user/checkout", label: "Оплата", icon: "credit-card" },
  { href: "/demo/user/settings", label: "Настройки", icon: "settings" },
  { href: "/demo/user/help", label: "Помощь", icon: "life-buoy" },
];
...
function UserMobileTabbarInner() {
  ...
  useEffect(() => {
    ...
    if (active) {
      ...
      btnRefs.current[key]?.scrollIntoView({
        behavior: "smooth",
        inline: "center",
        block: "nearest",
      });
  ...
  return (
    <div
      ...
      role="navigation"
      aria-label="Нижнее меню пользователя"
    >
      <div className="relative">
        <div
          aria-hidden
          className="pointer-events-none absolute inset-y-0 left-0 w-6 bg-gradient-to-r from-black/60 to-transparent"
        />
        <div
          aria-hidden
          className="pointer-events-none absolute inset-y-0 right-0 w-6 bg-gradient-to-l from-black/60 to-transparent"
        />
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  active &&
                    "border-white/20 bg-white/16 shadow-[0_0_0_1px_rgba(255,255,255,0.14)_inset]"
                )}
              >
                ...
                {active && (
                  <span
                    className="ml-auto inline-block h-1.5 w-1.5 rounded-full bg-white/90"
                    aria-hidden
                  />
                )}
              </Link>
...
const UserMobileTabbar = memo(UserMobileTabbarInner);
export default UserMobileTabbar;