...
export default function AdminServiceCategoryDetailPage() {
  ...
  const { servicesInCat, activeCount, draftCount, archivedCount } = useMemo(() => {
    const list = ADMIN_SERVICES.filter((s) => s.categoryId === cat.id);
      ...
      <header className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div className="min-w-0">
          <nav className="text-xs text-white/60 flex flex-wrap items-center gap-x-1 gap-y-0.5">
            ...
            <span className="opacity-50">/</span>
            <span className="text-white/80 truncate">{cat.name}</span>
          </nav>
          <h1 className="mt-1 text-2xl md:text-3xl font-semibold tracking-tight">{cat.name}</h1>
          <div className="mt-1 text-xs text-white/60">
            slug: <span className="font-mono">{cat.slug}</span> • ID:{" "}
      ...
      <section className="rounded-2xl border border-white/15 bg-white/[0.05] p-4 md:p-5">
        ...
        <div className="mt-2 grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            ...
            <div className="mt-1 text-lg font-semibold">{fmt(servicesInCat.length)}</div>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            <div className="text-xs text-white/60">Активных</div>
            <div className="mt-1 text-lg font-semibold">{fmt(activeCount)}</div>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            <div className="text-xs text-white/60">Черновики</div>
            <div className="mt-1 text-lg font-semibold">{fmt(draftCount)}</div>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            <div className="text-xs text-white/60">Архив</div>
            <div className="mt-1 text-lg font-semibold">{fmt(archivedCount)}</div>
          </div>
        </div>
