...
type Row = {
  ...
  name: string;
  url: string;
  count24h?: number;      // опционально — чтобы не падать на странных данных
  failPct: string;        // например "1.4%"
  retryPct: string;       // например "0.6%"
  latencyMed: number;     // ms
};
    ...
    <section className="rounded-2xl border border-white/15 bg-white/[0.05] p-3 md:p-4">
          ...
          <div key={r.id} className="rounded-2xl border border-white/15 bg-white/[0.04] p-3">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <div className="text-xs text-white/60 break-all">{r.url}</div>
              </div>
              <div className="text-xs text-white/60 text-right whitespace-nowrap">
                {fmtNum(r.count24h)} / 24ч
              </div>
            </div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
              ...
              <span className="font-medium">{r.latencyMed} ms</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="hidden md:block overflow-x-auto rounded-xl border border-white/10">
        <table className="min-w-[820px] w-full text-sm table-fixed">
          {/* генерируем colgroup без «сырых» пробелов — стабильный SSR */}
          <colgroup>
            {colWidths.map((w, i) => (
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-white/10 hover:bg-white/[0.03]">
                ...
                <td className="p-2">
                  <div className="truncate max-w-[320px]" title={r.url}>
                    {r.url}
                  </div>
                </td>
                ...
                <td className="p-2 whitespace-nowrap">{r.latencyMed} ms</td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr>
