...
import ConflictsPanel, {
  computeConflicts,
} from "@/app/demo/admin/booking/components/grid/ConflictsPanel";
import {
  ADMIN_RESOURCES,
  type AdminResource,
  type AdminTemplate,
...
export default function WeekGrid({
  ...
  const totalTimeRows = React.useMemo(
    () => ((HOUR_TO - HOUR_FROM) * 60) / MINUTE_STEP + 1,
    []
  );
  ...
  const [drag, setDrag] =
    React.useState<null | { dayIdx: number; from: string; to: string }>(null);
  const [openTpl, setOpenTpl] =
    React.useState<null | { dayIdx: number; from: string; to: string }>(null);
  ...
  function laneStyles(
    ...
    return {
      ...
      width: `calc(${pct}% - ${
        (LANE_GAP_PX * (lanesTotal - 1)) / lanesTotal
      }px)`,
    };
  }
  ...
  function MobileDayCard({ day, dayIdx }: { day: Date; dayIdx: number }) {
    ...
    const dayTemplates: LaneItem<AdminTemplate>[] = templates
      ...
      .map((t) => {
        ...
        toM = Math.max(
          fromM + MINUTE_STEP,
          Math.min((HOUR_TO - HOUR_FROM) * 60, toM)
        );
        return {
          id: (t as any).id ?? `${dayIdx}-${fromA}-${toA}`,
          fromM,
          toM,
          payload: t,
        };
      ...
      <div className="rounded-2xl border border-white/15 bg-white/[0.05] p-2">
        ...
        >
          <div
            className={`text-sm font-medium ${
              today ? "bg-white/5 rounded px-1.5 py-0.5" : ""
            }`}
          >
            {label}
          </div>
        ...
        >
          ...
          {Array.from({ length: totalTimeRows }).map((_, rIdx) => (
            ...
            >
              {drag && drag.dayIdx === dayIdx && (
                <div className="absolute inset-0 bg-emerald-400/15 pointer-events-none" />
              )}
            </div>
          ))}
            ...
            return (
              <button
                ...
                style={{
                  ...
                  ...laneStyles(li.lane, li.lanesTotal),
                }}
                title={label}
                aria-label={label}
                onClick={() =>
                  setOpenTpl({ dayIdx, from: t.from ?? "10:00", to: t.to ?? "11:00" })
                }
              >
          ...
          {exceptions.map((ex) => {
            ...
            const title = (ex as any).reason ?? "Исключение";
            return (
              <button
                key={(ex as any).id}
    ...
    return (
      <div className="overflow-x-auto">
        ...
        >
            ...
            Array.from({ length: totalTimeRows }).map((__, rIdx) => (
              <div
                ...
                style={{
                  ...
                  gridRow: `${rIdx + 2} / span 1`,
                }}
                onMouseDown={() => cellMouseDown(dayIdx, rowIdxToTime(rIdx))}
                onMouseEnter={() => cellMouseEnter(dayIdx, rowIdxToTime(rIdx))}
                onMouseUp={cellMouseUp}
                aria-label="Клетка сетки расписания"
              >
                {drag && drag.dayIdx === dayIdx && (
                  <div className="absolute inset-0 bg-emerald-400/15 pointer-events-none" />
                )}
          ...
          {/* Reservations — можно тоже разложить по lanes при желании */}
          {reservations.map((r) => {
            ...
            let toM =
              (endMinLocal ?? startMinLocal + MINUTE_STEP) - HOUR_FROM * 60;
            ...
            const timeLabel = new Date(startISO).toLocaleTimeString("ru-RU", {
              hour: "2-digit",
              minute: "2-digit",
            });
  ...
  return (
    <>
      ...
      {/* ВАЖНО: передаём resources в ConflictsPanel */}
      <ConflictsPanel conflicts={conflicts} resources={resources} />
