...
const STEP_MIN = 15; // единый шаг проверки перегрузки, мин.

function isValidHHMM(t?: string): t is string {
...
function isoDow(d: Date): number {
  const js = d.getDay(); // 0..6 (0 — воскресенье)
  return js === 0 ? 7 : js; // 1..7 (1 — понедельник)
}
...
/**
 * Вычисляет конфликты перегрузки по неделе (понедельник weekStart).
 * Конфликт — когда суммарная параллельность активных правил для ресурса > его capacity.
 */
export function computeConflicts({
  ...
  for (let offset = 0; offset < 7; offset++) {
    ...
    // фильтруем шаблоны, живущие в этот день
    let dayRules = templates.filter((t) => {
      if (!t?.active) return false;
      if (!t?.days?.includes(dow)) return false;
      if (!isValidHHMM(t.start) || !isValidHHMM(t.end)) return false;
    ...
    // рабочий диапазон дня по активным правилам
    const starts = dayRules.map((r) => toMin(r.start)).filter(Number.isFinite) as number[];
    const ends = dayRules.map((r) => toMin(r.end)).filter(Number.isFinite) as number[];
    if (starts.length === 0 || ends.length === 0) continue;

    const minStart = Math.min(...starts);
    const maxEnd = Math.max(...ends);

    // ограничим разумным временем: 06:00..23:00
    const dayFrom = Math.max(minStart, 6 * 60);
    const dayTo = Math.min(maxEnd, 23 * 60);
    if (!(dayFrom < dayTo)) continue;

    // группируем по ресурсу (если ресурс не указан — правило не участвует в перегрузке по конкретному ресурсу)
    const byResource = new Map<string, SlotTemplate[]>();
    for (const rule of dayRules) {
      const resIds = rule.resourceIds?.length ? rule.resourceIds : [];
      if (resIds.length === 0) continue; // пропускаем «безресурсные» правила для расчёта перегрузки
    ...
    byResource.forEach((rules, resId) => {
      const res =
        resources.find((r) => r.id === resId) ?? ADMIN_RESOURCES.find((r) => r.id === resId);
      const capacity = Math.max(1, res?.capacity ?? 1);

      for (let m = dayFrom; m < dayTo; m += STEP_MIN) {
        let activeLoad = 0;
  ...
  // дедуп + сортировка: дата, время, ресурс
  const uniq = [...new Map(out.map((c) => [c.id, c])).values()];
  return uniq.sort((a, b) => {
    if (a.date !== b.date) return a.date.localeCompare(b.date);
    const ta = Number(a.id.split("-").pop()) || 0;
    const tb = Number(b.id.split("-").pop()) || 0;
    if (ta !== tb) return ta - tb;
    return a.resourceId.localeCompare(b.resourceId, "ru");
  });
}
...
export default function ConflictsPanel({
  ...
  if (!conflicts?.length) {
    return (
      <div className="mt-2 text-xs text-white/60" role="status" aria-live="polite">
        Конфликтов не обнаружено.
      </div>
    );
  }

  const resById = React.useMemo(() => new Map(resources.map((r) => [r.id, r])), [resources]);

  return (
    <section
      className="mt-3 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-3"
      aria-labelledby="conflicts-title"
    >
      <div id="conflicts-title" className="text-sm font-medium mb-2">
      ...
      <ul className="space-y-2 text-xs">
        {conflicts.slice(0, 12).map((c) => {
          ...
          // Мягкая подсветка категории (при наличии связки сервис -> категория в словарях)
          ...
          return (
            <li
              key={c.id}
              className="rounded-lg bg-amber-400/5 border border-amber-400/20 p-2"
              aria-label={`Конфликт ${c.date}, ресурс ${resName}: ${c.reason}`}
            >
      ...
      {conflicts.length > 12 && (
        <div className="mt-2 text-[11px] text-white/70" aria-live="polite">
          … и ещё {conflicts.length - 12}
        </div>
      )}
    </section>
  );
