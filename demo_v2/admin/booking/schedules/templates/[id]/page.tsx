...
// строгая проверка HH:MM (00:00–23:59)
...
export default function AdminTemplateEditPage() {
  ...
  if (!current) {
    return (
      <div className="rounded-2xl border border-white/15 bg-white/[0.05] p-4">
        ...
        <div className="mt-3">
          <button
            onClick={() => router.push(`${base}/booking/schedules/templates`)}
  ...
  function onSave() {
    ...
    // бережно мерджим, чтобы не потерять возможные расширения схемы
    const patch: Partial<AdminTemplate> & Record<string, any> = {
      id: current.id,
      name: name.trim(),
      dayOfWeek,
      from,
      ...
      active: active === "true",
      // совместимость: кладём и parallelSlots, и parallel
      parallelSlots: Number(parallel) || 1,
    ...
    router.push(`${base}/booking/schedules/templates`);
  }
