// app/demo/admin/orders/components/AdminOrdersTable.tsx
"use client";
...
function statusTone(s: string) {
  ...
  if (["paid", "completed", "done", "delivered", "success", "shipped"].includes(v))
          ...
          <tbody>
            {rows.map((o) => (
              <tr
                ...
                tabIndex={0}
                onKeyDown={(e) => {
                  if (e.key === "Enter") router.push(`${base}/orders/${o.id}`);
                }}
              ...
              >
                ...
                <td className="p-3 align-top">
                  <div className="flex flex-wrap items-center justify-end gap-2">
                    ...
                    <Link
                      href={`${base}/orders/${o.id}/invoice?print=1`}
                      prefetch={false}
                      className="inline-flex items-center gap-1 rounded-lg border border-white/15 bg-white/10 px-2.5 py-1.5 hover:bg-white/15"
                      title="Печать счёта"
          ...
          rows.map((o) => (
            <div key={o.id} className="p-3">
              <Link
                href={`${base}/orders/${o.id}`}
                prefetch={false}
                className="block rounded-xl border border-white/15 bg-white/5 p-3 hover:bg-white/10"
                aria-label={`Открыть заказ ${o.id}`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="font-medium break-all leading-tight">#{o.id}</div>
                    <div className="mt-0.5 text-[11px] text-white/60">{o.createdAt}</div>
