// app/demo/admin/orders/components/AdminOrderSummary.tsx
"use client";
...
export default function AdminOrderSummary({ order }: { order: AdminOrder }) {
  ...
  const discountExplicit =
    ((order as any).discountAmount as number | undefined) ??
    ((order as any).discount as number | undefined);
  const promoCode =
    ((order as any).promoCode as string | undefined) ??
    ((order as any).coupon as string | undefined);

  // Поддержим обе схемы: total ИЛИ amount
  const orderTotal =
    (Number.isFinite((order as any).total) ? (order as any).total : undefined) ??
    (Number.isFinite((order as any).amount) ? (order as any).amount : undefined);

  const amount = Number.isFinite(orderTotal) ? (orderTotal as number) : subtotal;

  let discount = Math.max(0, discountExplicit ?? 0);
  if (!discountExplicit && amount < subtotal) {
    // Если явной скидки нет, но итог меньше подитога — считаем разницу скидкой
