...
export default function SourcesTable() {
  ...
  const filtered = rows.filter((r) => {
    ...
    return (
      ...
      r.slug.toLowerCase().includes(x) ||
      r.channel.toLowerCase().includes(x)
    );
  });

  return (
    <section className="rounded-2xl border border-white/15 bg-white/[0.05] p-3 md:p-4 w-full max-w-full">
      ...
      <div className="grid gap-2 md:hidden">
          ...
          <div key={r.id} className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="text-xs text-white/60 break-words">slug: {r.slug}</div>
                <div className="mt-1 flex flex-wrap items-center gap-2 text-xs">
                  ...
                  <span className="px-2 py-1 rounded-md bg-white/10 text-white/70">
                    {typeof r.conv === "number" ? `Conv ${r.conv}%` : "—"}
                  </span>
                  {r.active ? (
        ...
        {filtered.length === 0 && (
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 text-center text-white/60">
        ...
        <table className="w-full text-sm table-fixed">
          ...
          <thead className="text-white/60">
            <tr className="border-b border-white/10">
              ...
              <th className="text-left py-2 pr-3">Канал</th>
              <th className="text-left py-2 pr-3">Конверсия</th>
              <th className="text-right py-2 pl-3">Статус / Действия</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((r) => (
              <tr key={r.id} className="border-b border-white/5 hover:bg-white/[0.03]">
                ...
                <td className="py-2 pr-3 break-words">{r.slug}</td>
                <td className="py-2 pr-3">
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={5} className="py-8 text-center text-white/50">
                  Источники не найдены. Создайте новый источник.
                </td>
              </tr>
            )}
          </tbody>
