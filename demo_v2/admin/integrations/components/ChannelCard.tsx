// app/demo/admin/integrations/components/ChannelCard.tsx
"use client";
...
export default function ChannelCard({ id }: Props) {
  ...
  // Мемоизируем поиск канала — без лишних вычислений/рендеров
  const ch = React.useMemo(() => channels.find((x) => x.id === id), [channels, id]);
  ...
  // Локальные поля для «тестового сообщения»
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 min-w-0">
          <div className="min-w-0">
            <h2 id="channel-title" className="text-lg md:text-xl font-semibold truncate">
              {ch.name}
            </h2>
            <div className="text-white/60 text-sm mt-0.5 break-words">
              {ch.provider} • {ch.type?.toUpperCase?.() ?? ch.type}
            </div>
          </div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 min-w-0">
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3 min-w-0">
            ...
            <div
              className="rounded-lg bg-black/20 p-2 text-xs text-white/80 max-h-60 overflow-auto"
              role="region"
        ...
        {/* Тех.инфо */}
        <dl className="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs text-white/60">
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.03] p-2">
            <dt className="sr-only">Провайдер</dt>
            <dd>
              Провайдер: <span className="text-white/85">{ch.provider}</span>
            </dd>
          </div>
          <div className="rounded-lg border border-white/10 bg-white/[0.03] p-2">
            <dt className="sr-only">Отправлено за 24 часа</dt>
            <dd>
              Отправлено 24ч: <span className="text-white/85">{ch.sent24h}</span>
            </dd>
          </div>
          <div className="rounded-lg border border-white/10 bg-white/[0.03] p-2">
            <dt className="sr-only">Ошибки за 24 часа</dt>
            <dd>
      ...
      <section className="rounded-2xl border border-white/15 bg-white/[0.05] p-4 md:p-5">
        ...
        <div className="mt-3">
          <button
            onClick={sendTest}
            disabled={!to || !msg}
            className="rounded-lg bg-white/90 text-black px-4 py-2 text-sm hover:bg-white disabled:opacity-60 disabled:cursor-not-allowed"
          >
            Отправить
          </button>
        </div>
