// app/demo/admin/integrations/components/ChannelModal.tsx
"use client";
...
export default function ChannelModal({ open, onClose, onSubmit, initial }: Props) {
  ...
  const [touched, setTouched] = useState<{ name?: boolean; provider?: boolean }>({});

  const dialogRef = useRef<HTMLDivElement | null>(null);
  const firstInputRef = useRef<HTMLInputElement | null>(null);

  // ВАЖНО: все хуки — до раннего return
  const isValid = useMemo(
    () => name.trim().length > 0 && provider.trim().length > 0,
    [name, provider]
  );

  // Предзаполнение и управление фоном при открытии
  useEffect(() => {
    if (!open) return;
  ...
  const submit = () => {
    ...
    onSubmit({ name: name.trim(), provider: provider.trim(), type, status, features: [], settings: {} });
    onClose();
  };

  const nameError = touched.name && !name.trim() ? "Укажите название канала" : "";
  const providerError = touched.provider && !provider.trim() ? "Укажите провайдера" : "";
  ...
  return (
    ...
    >
      <div
        ...
        aria-describedby="channel-modal-desc"
        className="w-[96%] max-w-lg rounded-2xl border border-white/15 bg-neutral-900 p-4 md:p-5 shadow-xl min-w-0"
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p id="channel-modal-desc" className="text-xs text-white/60 mt-0.5">
        ...
        >
          ...
          <label className="flex flex-col gap-1 text-xs text-white/60">
            ...
            <input
              ref={firstInputRef}
              className={`rounded-lg bg-white/10 border px-3 py-2 text-sm text-white outline-none focus:ring-2 focus:ring-white/25 ${
                nameError ? "border-rose-400/60" : "border-white/15"
              }`}
            ...
            />
            {nameError && (
              <span id="name-error" className="text-rose-300 text-[11px]">
                {nameError}
              </span>
            )}
          </label>
          ...
          <label className="flex flex-col gap-1 text-xs text-white/60">
            ...
            <input
              className={`rounded-lg bg-white/10 border px-3 py-2 text-sm text-white outline-none focus:ring-2 focus:ring-white/25 ${
                providerError ? "border-rose-400/60" : "border-white/15"
              }`}
            ...
            />
            {providerError && (
              <span id="provider-error" className="text-rose-300 text-[11px]">
                {providerError}
              </span>
            )}
          </label>
          ...
          <div className="mt-2 flex flex-col sm:flex-row gap-2 sm:justify-end">
            ...
            <button
              type="submit"
              disabled={!isValid}
              className="px-4 py-2 rounded-lg bg-white hover:bg-white/90 text-neutral-900 disabled:opacity-60 disabled:cursor-not-allowed"
