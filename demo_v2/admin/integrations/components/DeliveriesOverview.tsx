  ...
  const rows = React.useMemo(() => {
    return DELIVERIES.filter(
      (d) =>
        ...
        (webhookId ? d.webhookId === webhookId : true)
    );
  }, [status, webhookId]);
  ...
  return (
    <section className="rounded-2xl border border-white/15 bg-white/[0.05] p-3 md:p-4 w-full max-w-full overflow-x-hidden">
        ...
        {rows.map((d) => (
          <div key={d.id} className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
            <div className="flex items-start justify-between gap-2">
              ...
              <span
                className={`px-2 py-0.5 rounded-md text-[11px] uppercase ${statusBadge(d.status)}`}
                aria-label={`Статус: ${d.status}`}
              >
                {d.status}
              </span>
            </div>
            ...
            <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
              ...
              <div className="rounded-lg bg-white/[0.04] p-2 text-right">
                ...
                <div className="text-white/80 mt-0.5" aria-label={`Задержка ${d.latencyMs} миллисекунд, код ${d.responseCode}`}>
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="min-w-[880px] w-full text-sm" role="table" aria-label="Доставки вебхуков">
          <thead className="text-white/70">
            <tr className="bg-white/[0.04]">
              <th scope="col" className="text-left p-3">Время</th>
              <th scope="col" className="text-left p-3">Webhook</th>
              <th scope="col" className="text-left p-3">Событие</th>
              <th scope="col" className="text-left p-3">Статус</th>
              <th scope="col" className="text-right p-3">Latency</th>
              <th scope="col" className="text-right p-3">Код</th>
              <th scope="col" className="text-right p-3">Открыть</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((d) => (
              <tr key={d.id} className="border-t border-white/10 hover:bg-white/[0.03]">
                ...
                <td className="p-3 max-w-[260px]">
                  <Link
                    ...
                    title={d.webhookId}
                    aria-label={`Открыть вебхук ${d.webhookId}`}
                  >
                    {d.webhookId}
                  </Link>
                </td>
                ...
                <td className="p-3 text-right whitespace-nowrap" aria-label={`Задержка ${d.latencyMs} миллисекунд`}>
                  {d.latencyMs} ms
                </td>
                <td className="p-3 text-right" aria-label={`Код ответа ${d.responseCode}`}>
                  {d.responseCode}
                </td>
