// src/app/demo/manager/_parts/tokens.ts
export const T = {
  page: "grid gap-6",
  hero:
    "relative overflow-hidden rounded-2xl border border-white/15 bg-gradient-to-b from-white/10 via-white/5 to-white/10 p-4 md:p-6 backdrop-blur-sm shadow-xl",
  card:
    "rounded-2xl border border-white/15 bg-white/[0.05] p-4 md:p-5 backdrop-blur-sm shadow-md",
  soft:
    "rounded-2xl border border-white/10 bg-white/[0.04] p-3 md:p-4 backdrop-blur",
  chip:
    "inline-flex items-center gap-1 rounded-full border border-white/15 bg-white/[0.06] px-2 py-0.5 text-[11px] text-white/80",
  dim: "text-white/70",
  mut: "text-white/60",
  btn:
    "inline-flex items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/10 px-3 py-2 text-sm hover:bg-white/15 focus:outline-none focus:ring-2 focus:ring-white/30",
  btnPrimary:
    "inline-flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-medium text-black hover:bg-white focus:outline-none focus:ring-2 focus:ring-white/30 disabled:opacity-60",
  input:
    "w-full rounded-xl border border-white/12 bg-white/[0.05] px-3 py-2 text-sm text-white placeholder:text-white/55 focus:outline-none focus:ring-2 focus:ring-white/35 focus:border-white/30 transition",
};
