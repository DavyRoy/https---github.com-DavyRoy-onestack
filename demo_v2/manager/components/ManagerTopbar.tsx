// app/demo/manager/components/ManagerTopbar.tsx
"use client";
...
/**
 * ManagerTopbar — единый верхний бар в стиле админки,
 * адаптивный (мобильный заголовок + full-width десктоп).
 * Содержит переключатель ролей (для демо), ссылку на уведомления и профиль.
 */
export default function ManagerTopbar() {
  ...
  // Закрывать меню ролей при смене маршрута
  useEffect(() => setRolesOpen(false), [pathname]);

  // Определяем текущую роль исходя из URL
  ...
  return (
    <header className="sticky top-0 z-50">
      <div className="relative mx-auto flex h-[3.75rem] w-full items-center justify-between gap-3 px-3 sm:px-6 md:px-10">
        ...
        {/* Левый блок: «домой» в раздел менеджера */}
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          >
            ...
            <span className="hidden sm:inline">Demo • Manager</span>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {/* Уведомления (роут менеджера) */}
          <Link
            href="/demo/manager/notifications"
            prefetch={false}
          ...
          {/* Профиль (роут менеджера) */}
          <Link
            href="/demo/manager/profile"
            prefetch={false}
