// app/demo/manager/components/ManagerBreadcrumbs.client.tsx — titleMap из JSON
"use client";
...
const titleMap: Record<string, string> = (() => {
  const map: Record<string, string> = { demo: "Демо", manager: "Менеджер" };
...
export default function Breadcrumbs() {
  ...
  const crumbs = useMemo(() => {
    ...
    const managerIdx = parts.findIndex((p) => p === "manager");
    const head = managerIdx >= 0 ? parts.slice(0, managerIdx + 1) : parts;
    const tail = managerIdx >= 0 ? parts.slice(managerIdx + 1) : [];

    return [
      { href: "/demo", label: "Главная" },
      { href: "/demo/manager", label: "Менеджер" },
      ...tail.map((_, idx) => {
