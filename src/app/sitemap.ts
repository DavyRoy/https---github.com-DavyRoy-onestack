...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const lastModified = "2026-09-24T00:00:00Z";
  ...
  // Английские версии перечисляем через alternates — так поисковик видит пары страниц.
  return routes.map((route) => ({
    ...
    alternates: {
      languages: { ru: canonical(route.path, "ru"), en: canonical(route.path, "en") },
    },
  }));
}
