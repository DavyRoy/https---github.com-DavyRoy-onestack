...
// Компонент для карточки гражданина
const CitizenCard = ({ citizen, index, onCitizenClick }: { citizen: any; index: number; onCitizenClick?: (citizen: any) => void }) => {
  return (
    <motion.div 
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <motion.div 
            className="w-10 h-10 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm"
          ...
          <div>
            ...
            <div className="text-white/60 text-xs">{citizen.age} лет • {citizen.city}</div>
          </div>
        </div>
        <motion.div 
          className={`px-2 py-1 rounded-full text-xs border backdrop-blur-sm ${
            citizen.status === 'active' ? 'bg-green-500/20 text-green-400 border-green-500/30' :
            citizen.status === 'pending' ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30' :
            'bg-blue-500/20 text-blue-400 border-blue-500/30'
          }`}
          whileHover={{ scale: 1.05 }}
        >
          {citizen.status === 'active' ? 'Активен' : citizen.status === 'pending' ? 'В обработке' : 'На паузе'}
        </motion.div>
      </div>
      ...
      <div className="space-y-2 mb-3">
        <div className="flex justify-between text-white text-xs">
          <span>Категория</span>
          <span className="font-bold">{citizen.category}</span>
        </div>
        <div className="flex justify-between text-white text-xs">
          <span>Услуг получено</span>
          <span>{citizen.servicesReceived}</span>
        </div>
        <ProgressBar 
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="text-white/60 text-xs">Последняя активность:</div>
        <span className="text-white/60 text-xs">{citizen.lastActivity}</span>
      </div>
      
      <div className="flex flex-wrap gap-1">
        {citizen.tags.map((tag: string, tagIndex: number) => (
          <motion.span 
            key={tagIndex}
...
const citizenCategories = [
  {
    ...
    citizensCount: 2847,
    avgSatisfaction: 87,
    servicesPerCitizen: 3.2,
    growth: "+12%",
    popularCategories: ["Пенсионеры", "Инвалиды", "Многодетные", "Ветераны"],
    details: {
      activeClients: 2341,
      newThisMonth: 156,
      avgAge: 58,
  ...
  },
  {
    ...
    description: "Граждане с льготным статусом • Социальные гарантии • Специальные программы",
    value: "1,234",
    metric: "Льготников",
    color: "#060010",
    ...
    citizensCount: 1234,
    avgSatisfaction: 92,
    servicesPerCitizen: 4.5,
    growth: "+8%",
    popularCategories: ["Инвалиды I-II гр.", "Ветераны труда", "Дети-инвалиды", "Многодетные"],
    details: {
      activeClients: 1189,
      newThisMonth: 67,
      avgAge: 62,
  ...
  },
  {
    ...
    description: "Ожидающие предоставления услуг • Приоритетные направления • Сроки ожидания",
    value: "156",
    metric: "В очереди",
    color: "#060010",
    ...
    citizensCount: 156,
    avgSatisfaction: 65,
    servicesPerCitizen: 1.2,
    growth: "+5%",
    popularCategories: ["Социальный патронаж", "Медицинский уход", "Юридические услуги", "Психологическая помощь"],
    details: {
      activeClients: 156,
      newThisMonth: 23,
      avgAge: 61,
  ...
  },
  {
    ...
    description: "Архив всех обращений • Статистика услуг • Динамика удовлетворенности",
    value: "8,452",
    metric: "Обращений",
    color: "#060010",
    ...
    citizensCount: 2847,
    avgSatisfaction: 85,
    servicesPerCitizen: 3.0,
    growth: "+15%",
    popularCategories: ["Консультации", "Материальная помощь", "Социальные услуги", "Психологическая поддержка"],
    details: {
      activeClients: 2341,
      newThisMonth: 423,
      avgAge: 57,
...
const citizensData = [
  {
    ...
    details: {
      ...
      notes: "Требуется регулярный патронаж"
    }
  },
  {
    id: 2,
    ...
    details: {
      ...
      notes: "Активно участвует в программах реабилитации"
    }
  },
  {
    id: 3,
    ...
    details: {
      ...
      notes: "Воспитывает 3 детей, требует поддержки"
    }
  },
  {
    id: 4,
    ...
    details: {
      ...
      notes: "Требуется помощь в оформлении документов"
    }
  },
  {
    id: 5,
    ...
    details: {
      ...
      notes: "Активно ищет работу, требует помощи с детским садом"
    }
  },
  {
    id: 6,
...
const citizenMetrics = [
  { category: "Всего граждан", value: "2,847", trend: "up", color: "#3B82F6", icon: "👥" },
  { category: "Льготных категорий", value: "1,234", trend: "stable", color: "#10B981", icon: "🎗️" },
  { category: "В очереди", value: "156", trend: "down", color: "#F59E0B", icon: "⏳" },
  { category: "Новых в месяце", value: "89", trend: "up", color: "#8B5CF6", icon: "🆕" },
  { category: "Средняя удовлетворенность", value: "87%", trend: "up", color: "#06B6D4", icon: "⭐" },
  { category: "Обращений в месяц", value: "423", trend: "up", color: "#84CC16", icon: "📞" }
];

const popularCitizens = [
  {
    name: "Иванова М.П.",
    growth: "+15%",
    services: "8",
...
const categoryDistribution = [
  { name: "Пенсионеры", value: 38, color: `rgba(${COLORS.blue}, 0.8)` },
  { name: "Инвалиды", value: 25, color: `rgba(${COLORS.purple}, 0.8)` },
  { name: "Многодетные семьи", value: 15, color: `rgba(${COLORS.emerald}, 0.8)` },
  { name: "Ветераны", value: 12, color: `rgba(${COLORS.rose}, 0.8)` },
  { name: "Матери-одиночки", value: 6, color: `rgba(${COLORS.amber}, 0.8)` },
  { name: "Другие категории", value: 4, color: `rgba(${COLORS.gray}, 0.8)` }
];
...
export default function CitizensCatalog() {
  ...
  const filteredCitizens = citizensData.filter(citizen => {
    const matchesSearch = citizen.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         citizen.category.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         citizen.tags.some((tag: string) => tag.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${COLORS.primary}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              />
              <span className="text-white text-sm">База активна • 87% удовлетворенность</span>
            </div>
          </div>
        </div>
      </motion.header>
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
          ...
          <div className="card--border-glow relative overflow-hidden rounded-2xl sm:rounded-3xl border border-white/10 bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-lg p-4 sm:p-6 md:p-8">
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 sm:gap-6">
              <div className="flex-1">
                ...
                >
                  <span className="text-emerald-400">2,847 получателей услуг</span> • 6 категорий помощи • <span className="text-blue-400">87% удовлетворенность</span>
                </motion.p>
                <motion.div 
                ...
                >
                  <div className="flex items-center gap-2 text-white/70 text-xs sm:text-sm">
                    ...
                    <span>2,341 активных клиентов</span>
                  </div>
                  <div className="flex items-center gap-2 text-white/70 text-xs sm:text-sm">
                    <div className="w-2 h-2 rounded-full bg-blue-400" />
                    <span>156 в очереди на услуги</span>
                  </div>
                  <div className="flex items-center gap-2 text-white/70 text-xs sm:text-sm">
                    <div className="w-2 h-2 rounded-full bg-purple-400" />
                    <span>1,234 льготных категорий</span>
                  </div>
                </motion.div>
              </div>
              <motion.div 
              ...
              >
                ...
                <div className="text-white/60 text-xs sm:text-sm mt-1 sm:mt-2">Статус базы</div>
        ...
        {/* Основные метрики граждан с анимациями */}
        <motion.section 
          className="mb-6 sm:mb-8"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        {/* Bento Grid с категориями граждан */}
        <GlobalSpotlight
          gridRef={gridRef}
                  ...
                  <div className="card__content flex flex-col relative text-white z-10 flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:gap-3 mb-3 sm:mb-4">
                      <div className="text-center p-1.5 sm:p-2 bg-white/5 rounded-lg backdrop-blur-sm border border-white/5">
                        <div className="text-white font-bold text-sm">{category.avgSatisfaction}%</div>
                        <div className="text-white/60 text-xs">Удовлетворенность</div>
                      </div>
                      <div className="text-center p-1.5 sm:p-2 bg-white/5 rounded-lg backdrop-blur-sm border border-white/5">
                        <div className="text-yellow-400 font-bold text-sm">{category.servicesPerCitizen}</div>
                        <div className="text-white/60 text-xs">Услуг на человека</div>
                      </div>
                    </div>
                    ...
                    <div className="mt-auto">
                      <div className="text-white/60 text-xs mb-1 sm:mb-2">Основные категории:</div>
                      <div className="flex flex-wrap gap-1">
                        {category.popularCategories.slice(0, isMobile ? 2 : 4).map((cat, catIndex) => (
                          <motion.span 
                            key={catIndex}
          ...
          >
            <div className="card--border-glow relative overflow-hidden rounded-xl sm:rounded-2xl border border-white/10 bg-white/5 backdrop-blur-lg p-4 sm:p-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                <div>
                  <h3 className="text-base sm:text-lg font-semibold text-white mb-3 sm:mb-4">📈 Демографические показатели</h3>
                  <div className="space-y-3 sm:space-y-4">
                    <div>
                      <div className="flex justify-between text-white text-xs sm:text-sm mb-1 sm:mb-2">
                        <span>Активные клиенты</span>
                        <span>{selectedCategoryData.details.activeClients}</span>
                      </div>
                      <ProgressBar 
                        value={(selectedCategoryData.details.activeClients / selectedCategoryData.citizensCount) * 100} 
                        color="#10B981" 
                        height="6px" 
                      />
                    </div>
                    ...
                    <div>
                      <div className="flex justify-between text-white text-xs sm:text-sm mb-1 sm:mb-2">
                        <span>Соотношение М/Ж</span>
                        <span>{selectedCategoryData.details.maleFemaleRatio}</span>
                      </div>
                      <ProgressBar value={50} color="#8B5CF6" height="6px" />
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:gap-4 mb-3 sm:mb-4">
                    <div className="text-center p-2 sm:p-3 bg-white/5 rounded-lg">
                      <div className="text-white font-bold text-base sm:text-lg">{selectedCategoryData.details.avgAge}</div>
                      <div className="text-white/60 text-xs">Средний возраст</div>
                    </div>
                    <div className="text-center p-2 sm:p-3 bg-white/5 rounded-lg">
                      <div className="text-white font-bold text-base sm:text-lg">{selectedCategoryData.avgSatisfaction}%</div>
                      <div className="text-white/60 text-xs">Удовлетворенность</div>
                    </div>
                    <div className="text-center p-2 sm:p-3 bg-white/5 rounded-lg">
                      <div className="text-white font-bold text-base sm:text-lg">{selectedCategoryData.servicesPerCitizen}</div>
                      <div className="text-white/60 text-xs">Услуг на человека</div>
                    </div>
                    <div className="text-center p-2 sm:p-3 bg-white/5 rounded-lg">
                      <div className="text-white font-bold text-xs sm:text-sm">{selectedCategoryData.citizensCount}</div>
                  ...
                  <div className="p-2 sm:p-3 bg-green-500/10 rounded-lg border border-green-500/20">
                    ...
                    <div className="text-green-400/60 text-xs mt-1">
                      +{selectedCategoryData.avgSatisfaction - 85}% к общей удовлетворенности
                    </div>
                  </div>
                </div>
              </div>
        ...
        {/* Список граждан с улучшенным дизайном */}
        <motion.section
          className="mb-6 sm:mb-8"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          ...
          <div className="card--border-glow relative overflow-hidden rounded-xl sm:rounded-2xl border border-white/10 bg-white/5 backdrop-blur-lg p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6">
              <h2 className="text-lg sm:text-xl font-bold text-white mb-2 sm:mb-0">
                📋 Граждане {filteredCitizens.length > 0 && `(${filteredCitizens.length})`}
              </h2>
            ...
            {filteredCitizens.length > 0 ? (
              ...
              >
                ...
                <div className="text-white text-base sm:text-lg mb-1 sm:mb-2">Граждане не найдены</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
            ...
            <div className="card--border-glow relative overflow-hidden rounded-xl sm:rounded-2xl border border-white/10 bg-white/5 backdrop-blur-lg p-4 sm:p-6">
              <h3 className="text-base sm:text-lg font-bold text-white mb-3 sm:mb-4 flex items-center gap-2">
                <span>🔥</span>
                <span>Активные граждане</span>
              </h3>
              <div className="space-y-2 sm:space-y-3">
                {popularCitizens.map((citizen, index) => (
                  <motion.div 
                    key={index}
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-white/60 text-xs truncate">{citizen.services} услуг • {citizen.category}</div>
                    </div>
                    <div className="text-right ml-2">
                      <div className="text-green-400 text-xs sm:text-sm font-bold">{citizen.growth}</div>
                      <div className="text-white/60 text-xs">{citizen.satisfaction}</div>
                      <div className="text-white/40 text-xs">{citizen.lastActivity}</div>
                    </div>
                  </motion.div>
                ))}
              </div>
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div className="card--border-glow relative overflow-hidden rounded-xl sm:rounded-2xl border border-white/10 bg-white/5 backdrop-blur-lg p-4 sm:p-6">
              ...
              <div className="space-y-2 sm:space-y-3">
                <motion.div 
                  ...
                  whileHover={{ scale: isMobile ? 1 : 1.02 }}
                >
                  <div className="text-blue-400 font-bold text-xs sm:text-sm">423 обращения</div>
                  <div className="text-blue-400/60 text-xs">За текущий месяц</div>
                  <ProgressBar value={75} color="#3B82F6" showLabel={false} height="4px" />
                </motion.div>
                <motion.div 
                  className="p-2 sm:p-3 bg-green-500/10 rounded-lg border border-green-500/20 hover:bg-green-500/15 transition-colors"
                  whileHover={{ scale: isMobile ? 1 : 1.02 }}
                >
                  <div className="text-green-400 font-bold text-xs sm:text-sm">89 новых</div>
                  <div className="text-green-400/60 text-xs">Граждан в этом месяце</div>
                  <ProgressBar value={65} color="#10B981" showLabel={false} height="4px" />
                </motion.div>
                <motion.div 
                  className="p-2 sm:p-3 bg-amber-500/10 rounded-lg border border-amber-500/20 hover:bg-amber-500/15 transition-colors"
                  whileHover={{ scale: isMobile ? 1 : 1.02 }}
                >
                  <div className="text-amber-400 font-bold text-xs sm:text-sm">156 в очереди</div>
                  <div className="text-amber-400/60 text-xs">Ожидают услуг</div>
                  <ProgressBar value={35} color="#F59E0B" showLabel={false} height="4px" />
                </motion.div>
              </div>
            </div>
            ...
            <div className="card--border-glow relative overflow-hidden rounded-xl sm:rounded-2xl border border-white/10 bg-white/5 backdrop-blur-lg p-4 sm:p-6">
              <h3 className="text-base sm:text-lg font-bold text-white mb-3 sm:mb-4 flex items-center gap-2">
                <span>📞</span>
                <span>Отдел работы с гражданами</span>
              </h3>
              <div className="space-y-2 sm:space-y-3">
                <motion.div 
                ...
                >
                  ...
                  <div className="text-white/60 text-xs">Руководитель отдела</div>
                ...
                >
                  ...
                  <div className="text-white/60 text-xs">Специалист по работе с льготниками</div>
                ...
                >
                  ...
                  <div className="text-white/60 text-xs">Координатор очереди</div>
      ...
      {/* Модальное окно деталей гражданина */}
      <AnimatePresence>
        {isCitizenModalOpen && selectedCitizen && (
          ...
          >
            <motion.div
              ...
              style={{ 
                '--glow-color': selectedCitizen.color.replace('#', ''),
              ...
              <div className="p-4 sm:p-6 border-b border-white/10">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 sm:gap-3">
                    <motion.div 
                      className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-lg sm:text-xl"
                      whileHover={{ scale: 1.1, rotate: 5 }}
                    >
                      {selectedCitizen.initials}
                    </motion.div>
                    <div className="max-w-[200px] sm:max-w-none">
                      <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-white truncate">{selectedCitizen.name}</h2>
                      <p className="text-white/60 text-xs sm:text-sm truncate">{selectedCitizen.age} лет • {selectedCitizen.city}</p>
                    </div>
                  </div>
                  <button
                    onClick={closeCitizenModal}
              ...
              <div className="p-4 sm:p-6 overflow-y-auto max-h-[50vh] sm:max-h-[60vh]">
                <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
                  <div className="text-center p-2 sm:p-3 md:p-4 bg-white/5 rounded-lg sm:rounded-xl">
                    <div className="text-lg sm:text-xl md:text-2xl font-bold text-white mb-1 sm:mb-2">{selectedCitizen.servicesReceived}</div>
                    <div className="text-white/60 text-xs sm:text-sm">Услуг получено</div>
                  </div>
                  <div className="text-center p-2 sm:p-3 md:p-4 bg-white/5 rounded-lg sm:rounded-xl">
                    <div className="text-lg sm:text-xl md:text-2xl font-bold text-white mb-1 sm:mb-2">{selectedCitizen.satisfaction}%</div>
                ...
                <div className="mb-4 sm:mb-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                    <div className="bg-white/5 rounded-lg p-2 sm:p-3">
                      ...
                      <div className="text-white font-semibold text-sm sm:text-base">{selectedCitizen.details.phone}</div>
                    </div>
                    <div className="bg-white/5 rounded-lg p-2 sm:p-3">
                      <div className="text-white/60 text-xs mb-1">Адрес</div>
                      <div className="text-white font-semibold text-sm sm:text-base">{selectedCitizen.details.address}</div>
                    </div>
                    <div className="bg-white/5 rounded-lg p-2 sm:p-3">
                      <div className="text-white/60 text-xs mb-1">Дата рождения</div>
                      <div className="text-white font-semibold text-sm sm:text-base">{selectedCitizen.details.birthDate}</div>
                    </div>
                    <div className="bg-white/5 rounded-lg p-2 sm:p-3">
                      <div className="text-white/60 text-xs mb-1">СНИЛС</div>
                      <div className="text-white font-semibold text-sm sm:text-base">{selectedCitizen.details.snils}</div>
                    </div>
                  </div>
                </div>
                
                <div className="mb-4 sm:mb-6">
                  <h3 className="text-base sm:text-lg font-semibold text-white mb-2 sm:mb-3">История обслуживания</h3>
                  <div className="space-y-2 sm:space-y-3">
                    <div className="flex justify-between items-center p-2 sm:p-3 bg-white/5 rounded-lg">
                      <div>
                        <div className="text-white font-medium text-sm">Дата регистрации</div>
                        <div className="text-white/60 text-xs">{selectedCitizen.details.registrationDate}</div>
                      </div>
                      <div className="text-green-400 text-xs">Активен</div>
                    </div>
                    <div className="flex justify-between items-center p-2 sm:p-3 bg-white/5 rounded-lg">
                      <div>
                        <div className="text-white font-medium text-sm">Последняя услуга</div>
                        <div className="text-white/60 text-xs">{selectedCitizen.details.lastService}</div>
                      </div>
                      <div className="text-blue-400 text-xs">{selectedCitizen.lastActivity}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-4 sm:mb-6">
                  ...
                  <div className="text-white font-semibold text-sm sm:text-base">{selectedCitizen.details.notes}</div>
                </div>
                
                <ProgressBar value={selectedCitizen.satisfaction} label="Удовлетворенность услугами" color={selectedCitizen.color} height="6px" />
                
                <div className="mt-4 sm:mt-6">
                  <div className="text-white/60 text-xs sm:text-sm mb-1 sm:mb-2">Категории и теги</div>
                  <div className="flex flex-wrap gap-1 sm:gap-2">
                    {selectedCitizen.tags.map((tag: string, index: number) => (
                      <span 
                        key={index}
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-4 text-white/60 text-xs sm:text-sm">
            <div className="flex items-center gap-2 sm:gap-4 flex-wrap justify-center">
              <span>© 2024 База граждан "Забота"</span>
            ...
            <div className="flex items-center gap-2 sm:gap-4 flex-wrap justify-center">
              ...
              <span>•</span>
              <span>87% удовлетворенность</span>
            </div>
          </div>
        </div>
      </motion.footer>
