...
const ROLES = [
  ...
  },
  {
    id: 'owner',
    label: 'Владелец',
    href: '/demo/social/owner',
    icon: '🏛️',
    description: 'Управление социальной организацией',
    color: 'from-indigo-500 to-purple-500'
  }
];

// Конфигурация меню для владельца социальных услуг
...
const RoleSwitcher = () => {
  ...
  const currentRole = useMemo(() => {
    if (pathname.includes('/owner')) return ROLES[2]; // owner
...
const MobileRoleSwitcher = () => {
  ...
  return (
    <div className="relative">
      {/* Кнопка переключения ролей в мобильном заголовке */}
      <motion.button
        onClick={() => setIsOpen(!isOpen)}
        className={cn(
        ...
        whileTap={{ scale: 0.98 }}
      >
        <div className={`w-6 h-6 rounded-lg bg-gradient-to-br ${currentRole.color} flex items-center justify-center text-white shadow-lg`}>
      ...
      {/* Выпадающий список ролей для мобильной версии */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            initial={{ opacity: 0, y: 10, scale: 0.95 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 10, scale: 0.95 }}
            className="absolute top-full right-0 mt-2 w-48 bg-black/90 backdrop-blur-2xl border border-white/30 rounded-xl p-2 shadow-2xl z-50"
          >
            {ROLES.map((role, index) => (
              <motion.button
                key={role.id}
                ...
                whileHover={{ x: 4, scale: 1.02 }}
              >
                <div className={`w-6 h-6 rounded-lg bg-gradient-to-br ${role.color} flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                  <Icon name={role.icon} size={12} />
                </div>
                <div className="flex-1 text-left min-w-0">
                  <div className="font-medium truncate text-xs">{role.label}</div>
                  <div className="text-xs text-white/70 truncate">{role.description}</div>
                </div>
                {currentRole.id === role.id && (
                  <motion.div
    ...
    <div className="hidden lg:fixed lg:inset-y-0 lg:z-40 lg:flex lg:w-80 lg:flex-col">
      ...
      >
        ...
        <motion.div 
          ...
          whileHover={{ y: -1 }}
          onClick={() => router.push('/demo/social/owner/modules/profile')}
        >
