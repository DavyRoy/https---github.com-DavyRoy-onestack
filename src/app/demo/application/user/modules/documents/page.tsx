...
const BentoCardGrid: React.FC<{
  ...
  return (
    <motion.div
      className={`bento-section ${gridClass} p-3 sm:p-4 lg:p-6 max-w-7xl 2xl:max-w-[1800px] mx-auto select-none relative`}
      ref={gridRef}
...
function DocumentCategoryCard({ category, onCardClick }: { category: DocumentCategory; onCardClick: () => void }) {
  ...
  return (
    <AdvancedParticleCard
      className="flex flex-col justify-between relative min-h-[200px] w-full max-w-full p-4 sm:p-6 rounded-2xl border border-white/10 font-light overflow-hidden transition-all duration-300 ease-in-out card--border-glow"
      style={{
    ...
    >
      <div className="h-full flex flex-col cursor-pointer">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3 flex-grow">
            <motion.div 
              ...
              whileHover={{ scale: isMobile ? 1 : 1.3, rotate: isMobile ? 0 : 5 }}
              transition={{ duration: 0.3, type: "spring" }}
            >
              {category.icon}
            </motion.div>
            <div className="flex-grow min-w-0">
              <h3 className="font-semibold text-white text-base sm:text-lg mb-1 sm:mb-2 leading-tight">
                {category.title}
              </h3>
          ...
          <motion.div 
            className="flex items-center gap-1 text-xs px-2 py-1 rounded-full border backdrop-blur-sm bg-white/10 text-white/80 border-white/20 flex-shrink-0"
        ...
        <div className="mb-4">
          <div className="text-white/70 text-xs sm:text-sm mb-2">Возможности:</div>
          <div className="flex flex-wrap gap-1">
            {category.features.slice(0, isMobile ? 2 : 3).map((feature, index) => (
              <span
                key={index}
        ...
        <motion.button
          className="w-full py-2 sm:py-3 px-3 sm:px-4 rounded-xl font-medium text-white text-xs sm:text-sm backdrop-blur-sm border transition-all duration-300 mt-auto"
          style={{
    ...
    >
      <div className="h-full flex flex-col relative z-10">
        ...
        <div className="flex gap-1 mt-2">
          {form.requirements.slice(0, 2).map((requirement, index) => (
            <span
              key={index}
  ...
  return (
    ...
    >
      <div className="h-full flex flex-col items-center justify-center text-center p-2">
        ...
        <h3 className="text-white font-semibold text-sm mb-1 leading-tight line-clamp-2">
          {action.label}
        </h3>
        {action.description && (
...
export default function DocumentsPage() {
  ...
  const stats = useMemo(() => ({
    ...
    availableForms: applicationForms.filter(form => form.status === 'available').length
  }), []);

  const filteredDocuments = useMemo(() => 
    documents.filter(document => document.type === activeCategory),
  [activeCategory]);
  ...
  const handleDocumentClick = (document: Document) => {
    ...
    setModalState(previous => ({ ...previous, documentDetails: true }));
  };

  const handleFormClick = (form: ApplicationForm) => {
    setSelectedForm(form);
    setModalState(previous => ({ ...previous, applicationForm: true }));
  };

  const closeModal = (modalName: keyof typeof modalState) => {
    setModalState(previous => ({ ...previous, [modalName]: false }));
  };

  // Адаптивная разметка
  const getGridColumns = (type: 'categories' | 'documents' | 'actions') => {
    switch (type) {
      case 'categories':
        return {
          mobile: 1,
      ...
      default:
        return { mobile: 1, tablet: 2, desktop: 3, large: 4 };
    }
  };
      ...
      <main className="max-w-7xl 2xl:max-w-[1800px] mx-auto px-3 sm:px-4 py-4 sm:py-6">
        ...
        >
          <div className="card--border-glow relative overflow-hidden rounded-2xl sm:rounded-3xl border border-white/10 bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-lg p-4 sm:p-6 lg:p-8">
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 sm:gap-6">
              <div className="flex-1 min-w-0">
                ...
                >
                  ...
                  <span className="line-clamp-2">Личные документы</span>
                </motion.h1>
                <motion.p 
                  className="text-white/60 text-sm sm:text-base lg:text-lg mb-3 sm:mb-4 line-clamp-2"
                  initial={{ opacity: 0, x: -30 }}
                  animate={{ opacity: 1, x: 0 }}
                ...
                <motion.div 
                  className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 mb-4 sm:mb-0"
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: 0.4 }}
                >
                  {[
                    { label: 'Всего документов', value: stats.totalDocuments.toString(), color: 'green-400' },
                    { label: 'Активных', value: stats.activeDocuments.toString(), color: 'blue-400' },
                    { label: 'Доступных форм', value: stats.totalForms.toString(), color: 'purple-400' },
                    { label: 'Истекших', value: stats.expiredDocuments.toString(), color: 'cyan-400' }
                  ].map((stat, index) => (
                    <div key={index} className="text-center p-2 sm:p-3 bg-white/5 rounded-lg border border-white/5">
                      <div className={`text-${stat.color} font-bold text-base sm:text-xl`}>{stat.value}</div>
                      <div className="text-white/60 text-xs line-clamp-2">{stat.label}</div>
                    </div>
                  ))}
                </motion.div>
              </div>
              ...
              <motion.div 
                ...
                transition={{ delay: 0.3 }}
              >
                <div className="text-white font-bold text-xl sm:text-2xl bg-gradient-to-r from-emerald-500 to-green-500 rounded-2xl px-4 sm:px-6 py-2 sm:py-3 shadow-lg inline-block">
            ...
            {/* Адаптивная навигация по категориям */}
            <motion.div 
            ...
            >
              ...
              <div className="flex gap-2 sm:gap-3 w-full sm:w-auto">
                <motion.button
                  className="flex-1 sm:flex-none px-3 sm:px-5 py-1.5 sm:py-2.5 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-white/80 hover:text-white transition-all duration-300 text-xs sm:text-sm font-medium backdrop-blur-sm flex items-center justify-center gap-1 sm:gap-2"
                  whileHover={{ scale: isMobile ? 1 : 1.05 }}
                  whileTap={{ scale: 0.95 }}
                >
                  <span className="line-clamp-1">📤 Экспорт архива</span>
                </motion.button>
                <motion.button
                  className="flex-1 sm:flex-none px-3 sm:px-5 py-1.5 sm:py-2.5 rounded-xl bg-blue-500/20 hover:bg-blue-500/30 border border-blue-500/30 text-blue-400 transition-all duration-300 text-xs sm:text-sm font-medium backdrop-blur-sm flex items-center justify-center gap-1 sm:gap-2"
                  whileHover={{ scale: isMobile ? 1 : 1.05 }}
                  whileTap={{ scale: 0.95 }}
                >
                  <span className="line-clamp-1">📎 Загрузить</span>
                </motion.button>
              </div>
            </motion.div>
          </div>
        ...
        >
          ...
          <BentoCardGrid gridRef={gridReference} columns={getGridColumns('categories')}>
            {documentCategories.map((category) => (
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
                ...
                <div className="flex items-center justify-between mb-4 sm:mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl font-bold text-white line-clamp-1">
                      {documentCategories.find(category => category.id === activeCategory)?.title}
                    </h2>
                  </div>
            ...
            <div className="space-y-4 sm:space-y-6">
                  ...
                  {documentCategories.map((category) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-white text-sm line-clamp-1">{category.title}</span>
                      </div>
              ...
              <div className="card--border-glow relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-lg p-4 sm:p-6 lg:p-8">
                ...
                <div className="space-y-3">
                  {documents
                    .filter(document => document.importance === 'high' || document.status === 'expired')
                    .slice(0, 3)
                    .map((document) => (
                      <motion.div
                        key={document.id}
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-red-400 text-sm font-semibold">
                            {document.status === 'expired' ? 'Истек' : 'Важно'}
                          </span>
                        </div>
                        <p className="text-white text-xs leading-tight line-clamp-2">{document.title}</p>
                        <p className="text-white/60 text-xs mt-1">{formatDate(document.dateCreated)}</p>
                      </motion.div>
                    ))}
                </div>
              </div>
        ...
        >
          <div className="card--border-glow relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-lg p-4 sm:p-6 lg:p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              {documents
                .sort((first, second) => new Date(second.dateCreated).getTime() - new Date(first.dateCreated).getTime())
                .slice(0, 5)
                .map((document) => (
                  <motion.div
                    key={document.id}
