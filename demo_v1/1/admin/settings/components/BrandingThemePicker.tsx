    ...
    <section className="grid gap-3 w-full max-w-full min-w-0">
      ...
      <div className="grid gap-2">
        ...
        >
          ...
          <div className="mt-3 text-[11px]" style={{ color: previewText, opacity: 0.6 }}>
...
function normalizeHex(s: string) {
  let v = s.trim();
  if (!v.startsWith("#")) v = `#${v}`;
  // простая валидация: #RGB или #RRGGBB
  if (/^#([0-9a-fA-F]{3}){1,2}$/.test(v)) return v.toLowerCase();
  return s; // оставим как есть, чтобы пользователь мог дописать
}
