...
function relativeFromNow(d: Date) {
...
export default function AdminOrderAudit({ order }: { order: AdminOrder }) {
  ...
  const [tick, setTick] = useState(0); // для автообновления относительного времени

  // автообновление «5 мин назад» каждые 60с, только когда включён тумблер
  useEffect(() => {
    if (!relative) return;
  ...
  const items = useMemo(() => {
    const raw = [...(order.audit ?? [])];
    raw.sort((a, b) => (b.at || "").localeCompare(a.at || "")); // новые сверху
  ...
  }, [order.audit, q, tick, relative]); // tick, relative — чтобы пересчитать отображение времени
  ...
  return (
    ...
    >
      ...
      <ol className="mt-3 space-y-2 text-sm" role="list">
        {items.map((a, i) => {
          const d = a.at ? parseDateSafe(a.at) : null;
