...
// ✅ общий каталог
import { Product } from "@/app/demo/(shared)/data/catalog/products.food";
...
export default function ProductTabs({
  ...
  // бейдж «Медиа»: считаем файлы, если они есть в демо-данных
  ...
  return (
    <section className="grid gap-3">
      ...
      {tab === "main" && <MainTab product={product} onDirty={onDirty} />}
      {tab === "variants" && <VariantsTab onDirty={onDirty} />}
      {tab === "price" && <PriceTab onDirty={onDirty} />}
      {tab === "inventory" && <InventoryTab onDirty={onDirty} />}
      {tab === "media" && <MediaTab onDirty={onDirty} />}
      {tab === "seo" && <SeoTab onDirty={onDirty} />}
      {tab === "attrs" && <AttrsTab onDirty={onDirty} />}
    </section>
  );
}
