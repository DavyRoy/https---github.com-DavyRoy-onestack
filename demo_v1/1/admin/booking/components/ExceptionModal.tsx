...
export default function ExceptionModal({
  ...
  }, [open, reason, type, fullDay, start, end]); // deps
  ...
  const timeIsValid =
    fullDay || (start && end && toMinutes(start) < toMinutes(end));
  ...
  function typeLabel(v: ExceptionType) {
    switch (v) {
      case "holiday":
        return "Праздник";
      case "blackout":
  ...
  function handleSubmit() {
    ...
    // Соберём человеко-читаемое описание в reason (API оставляем прежним)
    const parts = [
      `[${typeLabel(type)}]`,
  ...
  return (
    ...
    >
      <div
        ...
        aria-describedby="exception-desc"
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 id="exception-title" className="text-base md:text-lg font-semibold">
              Исключение / блэкаут
            </h2>
            <p id="exception-desc" className="mt-0.5 text-xs text-white/60">
        ...
        <div className="mt-4 flex flex-wrap justify-end gap-2">
          ...
          <button
            ...
            className="rounded-xl border border-red-400/40 bg-red-500/20 px-3 py-2 text-sm hover:bg-red-500/30 disabled:opacity-60"
          >
