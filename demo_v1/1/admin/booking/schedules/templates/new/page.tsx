...
import {
  ...
  ADMIN_RESOURCES,
  type AdminTemplate,
} from "@/app/demo/admin/booking/data/mockAdminBooking";

const DAY: Record<number, string> = { 0: "Пн", 1: "Вт", 2: "Ср", 3: "Чт", 4: "Пт", 5: "Сб", 6: "Вс" };
...
export default function TemplateNewPage() {
  ...
  function validate(): string | null {
    ...
    if (!TIME_RE.test(from) || !TIME_RE.test(to)) return "Некорректный формат времени (HH:MM).";
    if (from >= to) return "Время окончания должно быть позже начала.";
    if (!resourceId) return "Выберите ресурс.";
    if (parallelSlots < 1) return "Количество параллельных слотов должно быть ≥ 1.";
    return null;
  }
  ...
  function onSave() {
    ...
    router.replace(`/demo/admin/booking/schedules/templates/${id}`);
  }
    ...
    <div className="grid gap-6">
      ...
      <header className="rounded-2xl border border-white/15 bg-white/[0.05] p-4 md:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            ...
            <h1 className="mt-1 text-xl font-semibold tracking-tight sm:text-2xl">Новый шаблон слотов</h1>
            <p className="mt-1 text-sm text-white/70 sm:max-w-[70ch]">
              Повторяющееся окно доступности для ресурса/услуги. Сначала заполните основные поля — остальное можно поменять позже.
            </p>
          </div>
      ...
      <section className="rounded-2xl border border-white/15 bg-white/[0.05] p-4 md:p-5">
        <div className="mx-auto grid max-w-screen-sm gap-4 sm:max-w-screen-md">
          ...
          {/* День + Время — на мобилке в колонку */}
          ...
          {/* Ресурс / Услуга / Локация — мобильная колонка */}
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <label className="grid gap-1">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <label className="grid gap-1">
              ...
              <input
                ...
                min={1}
                value={parallelSlots}
                onChange={(e) => setParallelSlots(Math.max(1, Number(e.target.value) || 1))}
                className="rounded-xl bg-white/10 border border-white/15 px-3 py-2 text-sm"
              />
            </label>
          </div>
          ...
          {error && <div className="rounded-lg border border-red-400/40 bg-red-500/10 px-3 py-2 text-sm text-red-200">{error}</div>}

          {/* Bottom actions — на мобилке по 100% ширине */}
