...
export default function BundleItemsEditor({ initial }: { initial?: Bundle }) {
  ...
  const { serviceById, servicesByCategory, filteredCategoryIds } = useMemo(() => {
    const serviceById = new Map(ADMIN_SERVICES.map((s) => [s.id, s]));
    const servicesByCategory = new Map<string, typeof ADMIN_SERVICES>();
    for (const cat of SERVICE_CATEGORIES) {
      servicesByCategory.set(
        cat.id,
        ADMIN_SERVICES.filter((s) => s.categoryId === cat.id)
      );
    }

    // Фильтрация по поиску (в селекте)
    const needle = q.trim().toLowerCase();
    const filteredCategoryIds = SERVICE_CATEGORIES.map((c) => c.id).filter((catId) => {
      if (!needle) return true;
      const xs = servicesByCategory.get(catId) || [];
      return xs.some(
        (s) =>
    ...
    return { serviceById, servicesByCategory, filteredCategoryIds };
  }, [q]);
          ...
          <tbody>
            {items.length > 0 ? (
              items.map((it, i) => {
                ...
                return (
                  <tr key={`${it.serviceId}-${i}`} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="p-2">
                      <div className="flex flex-col">
                        ...
                        <div className="mt-1 text-[11px] text-white/50">
                          ...
                          {isDup && (
                            <span className="ml-2 text-rose-300">Дублируется</span>
                          )}
                        </div>
                      </div>
                    </td>
        ...
        <div className="md:hidden divide-y divide-white/10">
          {items.length === 0 ? (
            ...
            items.map((it, i) => {
              ...
              return (
                <div key={`${it.serviceId}-${i}`} className="p-3">
