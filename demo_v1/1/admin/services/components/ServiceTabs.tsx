...
export default function ServiceTabs({
  ...
  const [tab, setTab] = useState<Tab>(TABS.some(t => t.id === tabFromUrl) ? tabFromUrl : "main");
  ...
  const errors = useMemo(() => {
    const xs: string[] = [];
    if (!name.trim()) xs.push("Укажите название услуги.");
    if (name.trim().length > 120) xs.push("Название слишком длинное (макс. 120).");
    if (!slug.trim()) xs.push("Slug обязателен.");
    if (slug.length > 120) xs.push("Slug слишком длинный (макс. 120).");
    if (durationMin <= 0) xs.push("Длительность должна быть больше 0.");
    if (basePrice < 0) xs.push("Цена не может быть отрицательной.");
    if (promoPrice !== "" && Number(promoPrice) < 0) xs.push("Акционная цена не может быть отрицательной.");
    return xs;
    ...
    <section className="grid gap-3">
      ...
      <div className="sticky top-0 z-10 -mx-4 md:-mx-5 px-4 md:px-5 py-2 backdrop-blur-sm bg-black/10 border-b border-white/10">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar" role="tablist" aria-label="Вкладки услуги">
            {TABS.map((t, i) => (
      ...
      {tab === "main" && (
        <Section title="Основное">
            ...
            <label className="grid gap-1">
              ...
              {tagsParsed.length > 0 && (
                <div className="text-[11px] text-white/60">
                  Итог:{" "}
                  {tagsParsed.map((t) => (
                    <span key={t} className="inline-block rounded bg-white/10 px-1.5 py-0.5 mr-1">
                      {t}
                    </span>
                  ))}
                </div>
          ...
          <div className="text-xs text-white/60">
            Audit: изменено демо-пользователем 2025-09-10 12:00
          </div>
        </Section>
      )}
      ...
      {tab === "duration" && (
        <Section title="Длительность и слоты">
      ...
      {tab === "resources" && (
        <Section title="Исполнители/ресурсы (демо)">
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-white/15 bg-white/[0.05] p-4 md:p-5 grid gap-3">
      <div className="flex items-center justify-between gap-2">
